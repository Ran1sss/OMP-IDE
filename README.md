...
**OMP IDE 2.2.0** — десктопная рабочая среда для агент-харнесса
**Oh My Pi**: Monaco-редактор, PTY-терминал, Git, управление моделями и
AI-агент в одном окне. Агент работает прямо с воркспейсом, а Team Mode
автоматически подбирает роли, строит редактируемый план и запускает
изолированных OMP-воркеров с живым статусом и обязательной проверкой результата.
...
- **Explorer** — дерево файлов с live-обновлением (chokidar), git-статусы файлов, цветные пульсы правок (агент/вы/внешние)
- **Поиск по проекту** — ripgrep под капотом, замена по файлам с предпросмотром
- **Git-панель** — статус, стейджинг перетаскиванием, диффы, ветка в статус-баре
- **Встроенный терминал** — полноценный PTY (xterm.js + node-pty)
- **Командная палитра** — `Ctrl+Shift+P`, быстрое открытие файла — `Ctrl+P`
- **Переключатель воркспейсов** — чип в титлбаре: закреплённые и недавние папки, бейджи «сессия агента», открытие в новом окне
- **Интерактивный тур** — знакомство с IDE поверх живого интерфейса (18 шагов, RU/EN)
...
- **Team Mode** — auto-routing под задачу, редактируемая dispatch-карта и DAG зависимостей; роли coder/tester/reviewer выполняются в изолированных OMP-процессах параллельно там, где позволяет план, с handoff-заметками и финальной проверкой
- Зона «Сейчас»: живой срез — текущий инструмент, активная задача, диффстат сессии
- Чипы-упоминания файлов (drag&drop в промпт) и ✦ Prompt Improve (`Ctrl+E`)
- История сессий с группировкой по дням (только чтение)
...
- Задачи и steering из Telegram, transient typing во время работы, интерактивные вопросы и ровно одна компактная финальная сводка или ошибка; live Team-статус доступен через `/status`
- Слушатель групповых чатов с режимами диалога и очередью запросов «Запросы»

### UI и языки
- Дизайн-система Velvet Nebula: стеклянные слои, ambient-подсветка, дисциплина Motion (full/events/minimal)
- Полная локализация RU/EN с живым переключением — без перезапуска
